...
class ProxySetting(models.Model):
    ...
    proxy_desc = models.CharField(u"代理设置描述", max_length=55, default="反向代理")

    def __str__(self):
        return "|".join([self.load_balance_name, self.location_rx, self.proxy_pass, self.proxy_host, str(self.proxy_cache)] )
...
class WebServerHost(models.Model):
    ...
    def save(self, *args, **kwargs):
        try:
            super(WebServerHost, self).save(*args, **kwargs)
        finally:
            from phaser1.api.fmg.nginx_txt.v11.conf_v11 import get_vhost_configtxt, init_all_confgs
            init_all_confgs()

    def delete(self, *args, **kwargs):
        try:
            from phaser1.api.fmg.nginx_txt.v11.conf_v11 import delete_file_by_port
            delete_file_by_port(self)
        finally:
            super(WebServerHost, self).delete(*args, **kwargs)
