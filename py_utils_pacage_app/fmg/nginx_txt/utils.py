...
class NginxConf():
    ...
    def get_server_content(self):
        ...
        server_content += "\n" + NginxConf(cfg=origin_cfg).get_error_page_settings() +"\n"
    ...
    def get_location_conf(self, **kwargs):
        ...
        for location_cfg in kwargs["location_cfgs"]:
            ...
            ### 开始判断location的各种设置内容
            if "proxy" in location_cfg.keys():
                from .inintal_cfg import location_proxy
                custom_error="proxy_intercept_errors on;\n" if "custom_error" in location_cfg.keys() else ""
                proxy_cache = "proxy_cache {};".format(self.cache_name) if "proxy_cache" in location_cfg.keys() else ""
                start_cache_strategy = "" if ("cache_strategy" in location_cfg.keys() and proxy_cache != "") else "#"
            ...
            if "file_show_save_path" in location_cfg.keys():
                res_str = ("\n" + self.space + self.tpace * 2).join(
                    fileshow.format(file_show_save_path=location_cfg["file_show_save_path"] ).split("\n") )
                string_demo += res_str

            if "return_code" in location_cfg.keys():
                res_str = self.space + self.tpace * 2 +"return {};\n".format(location_cfg["return_code"])
                string_demo += res_str

            if "static" in location_cfg.keys():
                res_str = self.space + self.tpace * 2 + "root {};\n".format(location_cfg["site_path"])
                res_str += self.space + self.tpace * 2 + "index index.html index.htm index.php;\n"
                string_demo += res_str

            if "alias" in location_cfg.keys():
                res_str = self.space + self.tpace * 2 + "alias {};\n".format(location_cfg["alias"])
                string_demo += res_str
    ...
    def get_fdl_str(self):
        return "\n" + self.space + self.tpace + ("\n"+self.space+self.tpace).join(
            fdl_partern.format(**origin_cfg).split("\n")).replace("$%^", "{").replace("^%$","}")
